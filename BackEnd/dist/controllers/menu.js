"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const menu_1 = __importDefault(require("../models/menu"));
class MenuController {
    getMenus(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const menus = yield menu_1.default.findAll();
                res.json(menus);
            }
            catch (error) {
                res.status(500).json({ message: 'Hubo un error al obtener los menus.' });
            }
        });
    }
    createMenu(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { nombre, icono, accesoA } = req.body;
                const newMenu = yield menu_1.default.create({ nombre, icono, accesoA });
                res.json(newMenu);
            }
            catch (error) {
                res.status(500).json({ message: 'Hubo un error al crear el menu.' });
            }
        });
    }
    updateMenu(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { idMenu } = req.params;
                const { nombre, icono, accesoA } = req.body;
                yield menu_1.default.update({ nombre, icono, accesoA }, { where: { idMenu } });
                res.json({ message: 'Menu actualizado correctamente.' });
            }
            catch (error) {
                res.status(500).json({ message: 'Hubo un error al actualizar el menu.' });
            }
        });
    }
    deleteMenu(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { idMenu } = req.params;
                yield menu_1.default.destroy({ where: { idMenu } });
                res.json({ message: 'Menu eliminado correctamente.' });
            }
            catch (error) {
                res.status(500).json({ message: 'Hubo un error al eliminar el menu.' });
            }
        });
    }
}
exports.default = new MenuController();
